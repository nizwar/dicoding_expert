...
public class TvShowFragment extends Fragment {
    ...
    private void getTVs() {
        new DataObserver.LoadKatalogAsync(getActivity(), 1,null, new DataObserver.LoadKatalogsCallback() {
            @Override
            ...
            public void postExecute(ArrayList<Katalog> katalogs) {
                if (katalogs.size() > 0) {
                    recMovie.setAdapter(new CustomRecyclerAdapter(getActivity(), katalogs, 1));
                    showList();
                } else {
                    showEmpty();
                }
...
//    private void getTVsOld() {
//        pbLoading.setVisibility(View.VISIBLE);
//        recMovie.setAdapter(null);
//        ArrayList<Katalog> favorite = sqFavorite.getFavorite(1, null);
//        if (favorite.size() > 0) {
